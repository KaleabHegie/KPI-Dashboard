            ...
            success: function(data){
              ...
              if(data.kraz.length > 0){
                ...
                data.kraz.forEach((kra) =>{
                  let ministryImages = kra.responsible_ministries.map(ministry => `<img src="${ministry.image}" class="img-fluid rounded-circle me-1" style="width: 60px; height: 44px;" alt="ministry image">`).join(' '); // Create img elements for each ministry image
                  $("#kra-card-list").append(`
                      <h4 class="fw-bold  text-center pt-3">${kra.activity_name_eng}</h4>
                      ${ministryImages}
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        data.policy.forEach((item) => {
            ...
            let roundedNum1 = Math.round(item.score_card.avg_score);
            let progressBarColor = item.score_card.scorecard_color; // Get color from API
  
            cardMinistry += `
            <div  class="col-md-3  col-xl-2 d-none d-md-block ministry-card"
            data-id = ${item.id}
            data-name = "${item.code}"
            >
               <div style="height:110px;" class="card unique-card ${ item.id == 3 ? selectedCard : '' } selected-card social-widget-card bg-light">
                   <div class="card-body d-flex justify-content-between align-items-center p-2">
                       <div class="d-flex flex-column">
                           <h3 class="text-black m-0 small mb-1 ">${item.policyAreaEng} </h3>
                           <div class="progress mt-3">
                           <div class="progress-bar" role="progressbar" style="width: ${roundedNum1}%; background-color: ${progressBarColor};" aria-valuenow="${roundedNum1}" aria-valuemin="0" aria-valuemax="100">${roundedNum1}%</div>
                         </div>
                       </div>
                       <i class="fa fa-${item.icon.split(",")[1]}" style="font-size: 15px;"></i>
