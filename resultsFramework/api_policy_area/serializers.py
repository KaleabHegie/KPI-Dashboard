    ...
    def get_poor_performance(self, obj):
        ...
        if quarter and year:
            ...
            performance = obj.kra_goal.filter(
                            ...
                            indicators__quarter_indicators__year__year_amh=year,
                            indicators__quarter_indicators__quarter__quarter_eng=quarter,
                            indicators__quarter_indicators__quarter_performance__lt=0.5 * F('indicators__quarter_indicators__quarter_target')
                            ).aggregate(
                               low_performance=Coalesce(Count('indicators__quarter_indicators'), 0)
                            )['low_performance']
            return  {
                'performance': performance,
                'percentage' : (performance/ total_target) * 100 if total_target > 0 else 0
    ...
    def get_no_performance(self, obj):
        ...
        if quarter and year:
            ...
            performance = obj.kra_goal.filter(
                        ...
                        no_performance=Coalesce(Count('indicators__quarter_indicators'), 0)
                        )['no_performance']
            return  {
                'performance': performance,
                'percentage' : (performance/ total_target) * 100 if total_target > 0 else 0
    ...
    def get_count_indicator_target(self, obj):
        ...
        if year and quarter:
            return obj.policy_area_goal.aggregate(count_indicator_target=Count(
                    ...
                    filter=Q(kra_goal__indicators__quarter_indicators__quarter_target__isnull=False) & 
                           Q(kra_goal__indicators__quarter_indicators__year__year_amh=year) &
                           Q(kra_goal__indicators__quarter_indicators__quarter__quarter_eng=quarter)
                )
    ...
    def get_count_indicator_have_performance(self, obj):
        ...
        if year and quarter:
            return obj.policy_area_goal.aggregate(count_indicator_have_performance=Count(
                    'kra_goal__indicators',
                    filter=Q(kra_goal__indicators__quarter_indicators__quarter_target__isnull=False) & 
                           Q(kra_goal__indicators__quarter_indicators__quarter_performance__isnull=False) &
                           Q(kra_goal__indicators__quarter_indicators__year__year_amh=year) &
                           Q(kra_goal__indicators__quarter_indicators__quarter__quarter_eng=quarter)
                )
    ...
    def get_count_indicator_have_target_and_no_performance(self, obj):
        ...
        if quarter and year:
            return obj.policy_area_goal.aggregate(count_indicator_have_target_and_no_performance=Count(
                    'kra_goal__indicators',
                    filter=Q(kra_goal__indicators__quarter_indicators__quarter_target__isnull=False) & 
                           Q(kra_goal__indicators__quarter_indicators__quarter_performance__isnull=True) &
                           Q(kra_goal__indicators__quarter_indicators__year__year_amh=year) &
                           Q(kra_goal__indicators__quarter_indicators__quarter__quarter_eng=quarter)
                )
